...
function Employees() {
  ...
  const todayDate = () => {
    ...
    return `${newToday.getFullYear()}-${String(
      newToday.getMonth() + 1
    ).padStart(2, "0")}`;
  };
  ...
  const EmployeeTable = ({
    ...
    const filteredRecords = records.filter(
      (record) =>
        ...
        record.employee_id.toLowerCase().includes(filterText.toLowerCase()) ||
        record.role.toLowerCase().includes(filterText.toLowerCase())
    );
    ...
    const columns = [
      ...
      },
      {
        name: "Action",
        cell: (row) => (
          <div className="text-blue-600 space-y-1 text-center flex gap-2">
            <button
              onClick={() => {
                setId(row.id);
                setName(row.name);
                setShowSummary(true);
                hadleMonthSummary(row.id);
              }}
            ...
            <button
              ...
              className="hover:underline block"
            >
              <Trash2 size={20} className="text-blue-600 cursor-pointer" />{" "}
            </button>
          </div>
        ),
        ignoreRowClick: true,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-blue-100 p-6">
      ...
      <EmployeeTable
        ...
        hadleMonthSummary={hadleMonthSummary}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (value) {
                    ...
                    hadleMonthSummary(id, parseInt(year), parseInt(month));
                  }
                }}
              />
